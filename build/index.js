...
const mdFileList = fs.readdirSync(resolve('../'), { withFileTypes: true, encoding: 'utf-8' }).reduce((t, c) => {
...
function genMainFile() {
	...
	fs.readdirSync(pathStr, { withFileTypes: true, encoding: 'utf-8' }).forEach((item) => {
		if (!item.isFile() || !/\.json$/.test(item.name)) {
			return null;
		}
